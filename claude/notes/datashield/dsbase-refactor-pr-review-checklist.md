...
- [ ] **Error assertions test the *specific* server message via `DSI::datashield.errors()`, not just the generic wrapper.** When a client-side check moves server-side, the client only raises `"DataSHIELD errors"`; a bare `expect_error(…, "DataSHIELD errors")` no longer pins *which* error fired and would pass on the wrong failure. Prefer the pattern already in `test-arg-ds.dataFrameSubset.R`: trigger the error, then `res.errors <- DSI::datashield.errors()` and `expect_match(res.errors$sim1, "<the specific server-side message>")` per study. **Do this as a dedicated full pass once the whole batch is finished** — sweep every `test-arg-*` / `test-smk-*` that was reduced to a generic `"DataSHIELD errors"` match and tighten it against the real `datashield.errors()` text.
